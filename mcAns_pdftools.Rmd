...
pdf_path = "D:\\OneDrive - City, University of London\\@Courses\\BS2203\\! Exam, test/OnlineExam2020"
...
df_exam <- readxl::read_excel(
#  file.path(pdf_path, "FR2203 - Company Valuation Gradesheet - Spring 2020 (all sheets).xlsx"),
  file.path(pdf_path, "BS2203 - Financial Analysis Gradesheet - Spring 2020 (all sheets).xlsx"),
  range = paste0("marked!A1:Q", 1+length(files)))
...
# Note: Below is a flexible control to handle speciific student, if necessary, by commenting out "| TRUE"
  if (#studentID == 170042107 # (with non-A4-size pdf)    #170058149 
    ...
    marks <- df_exam %>% filter(student == studentID)
    
    # set the texts for the marks to print    
    MCQ = paste0("MC = ", pct(marks$MC), 
                 "% (", marks$countCor, " correct out of 20)")    
    Q21 = paste0("Q21 = ", pct(marks$Q21))
    Q22 = paste0("Q22 = ", pct(marks$Q22))
    divider = paste0("----------------")
    Total = paste0("Total = ", pct(marks$Grade), "%")
